...
class PacketWriter {
	...
	void startKeepAliveProcess(Socket socket) {
		this.socket = socket;
		...
		Log.e("XmppManager", "长连接时间间隔" + keepAliveInterval);
	...
	private class KeepAliveTask implements Runnable {
		...
		public void run() {
			try {
				...
				// heartbeats.
				// 这里的意思是第一次进入的时候要长久一点。
				// Log.e("KeepAliveTask", (15000 + delay)+"第一次保持的时间----"+new
				// Date().toLocaleString());
				...
				while (!done && keepAliveThread == thread) {
					Log.e("KeepAliveTask", "----------------------------"
							+ lastActive);
					Log.e("KeepAliveTask",
							"长连接保持中" + new Date().toLocaleString() + "----"
									+ socket.isConnected() + "----" + done
									+ "----" + (keepAliveThread == thread));

					synchronized (writer) {
						// Send heartbeat if no packet has been sent to the
						// server for a given time
						if (System.currentTimeMillis() - lastActive >= delay) {

							long fs = TrafficStats.getUidTxBytes(Process
									.myUid());
							long js = TrafficStats.getUidRxBytes(Process
									.myUid());
							Log.e("KeepAliveTask", js + "总发送：--" + fs);
							writer.write("i am lee");
							writer.flush();
							Log.e("KeepAliveTask",
									"接收---"
											+ (TrafficStats
													.getUidRxBytes(Process
															.myUid()) - js)
											+ "总发送：--"
											+ (TrafficStats
													.getUidTxBytes(Process
															.myUid()) - fs));
							Log.e("KeepAliveTask",
									"----------------------------");
						}
				...
				Log.e("KeepAliveTask", "我碰到io异常了" + e.toString());
				e.printStackTrace();
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				Log.e("KeepAliveTask", "我碰到异常了" + e.toString());
				e.printStackTrace();
			} finally {
				Log.e("KeepAliveTask", "客户端是连接的--" + socket.isConnected()+"---");
				try {
					socket.sendUrgentData(0xff);
				} catch (IOException e) {
				    Log.e("KeepAliveTask", "服务端的确断开了");
					e.printStackTrace();
				}
				// if(socket.isConnected()){
				//    startKeepAliveProcess(socket);
				// }
