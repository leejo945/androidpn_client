	...
	private class KeepAliveTask implements Runnable {
        ...
        public void run() {
            ...
            try {
            	...
            	//这里的意思是第一次进入的时候要长久一点。
            	//Log.e("KeepAliveTask",  (15000 + delay)+"第一次保持的时间----"+new Date().toLocaleString());
            ...
            while (!done && keepAliveThread == thread) {
             	Log.e("KeepAliveTask", "长连接保持中"+new Date().toLocaleString());
                ...
                synchronized (writer) {
                    ...
                    if (System.currentTimeMillis() - lastActive >= delay) {
                        try {
//                        	Log.e("KeepAliveTask","----------------------------");
//                            long fs =	  TrafficStats.getUidTxBytes(Process.myUid());
//                            long js =  TrafficStats.getUidRxBytes(Process.myUid());
//                        	Log.e("KeepAliveTask", js+"总发送：--"+fs);
                            writer.write("");
                            writer.flush();
//                            Log.e("KeepAliveTask", "接收---"+(TrafficStats.getUidRxBytes(Process.myUid())-js)+"总发送：--"+(TrafficStats.getUidTxBytes(Process.myUid())-fs));
//                            Log.e("KeepAliveTask","----------------------------");
                        }
                        catch (Exception e) {
                            // Do nothing
